...
#include <time.h>
#include <unistd.h>
#include <sys/file.h> // 파일 잠금(flock)을 위해 추가
#include <sys/stat.h> // 파일 정보(stat)를 얻기 위해 추가

// 방문자 카운터 파일을 정의합니다.
#define COUNTER_FILE "/var/www/html/counter.dat"
// 오늘 방문한 IP 주소를 기록할 파일을 정의합니다.
...
int main(void) {
    ...
    int new_visitor = 1; // 새로운 방문자인지 확인하는 플래그

    // 현재 날짜와 파일의 최종 수정 날짜를 비교하여 파일 초기화
    struct stat st;
    ...
    if (stat(UNIQUE_VISITORS_FILE, &st) == 0) {
        ...
        if (today->tm_year != file_date->tm_year ||
            today->tm_mon != file_date->tm_mon ||
            today->tm_mday != file_date->tm_mday) {
            // 날짜가 바뀌었으면 파일 초기화
    ...
    while (fgets(line, sizeof(line), fp_unique) != NULL) {
        ...
        if (remote_addr != NULL && strcmp(line, remote_addr) == 0) {
            new_visitor = 0; // 이미 방문한 IP
            break;
        }
    }

    // 새로운 방문자일 경우 카운트 증가 및 IP 기록
    if (new_visitor) {
        // 카운터 파일 열기 (읽기/쓰기 모드)
    ...
    printf("        body { font-family: 'Noto Sans KR', sans-serif; background-color: #f0f0f0; color: #333; line-height: 1.6; margin: 0; padding: 40px 20px; display: flex; flex-direction: column; min-height: 100vh; }\n");
    ...
    printf("        .footer-info { margin-top: auto; padding: 20px; background-color: #f0f0f0; border-top: 1px solid #ddd; color: #666; font-size: 0.9em; text-align: center; }\n");
    printf("        .footer-info p { margin: 5px 0; }\n");
    printf("        .counter-container { background-color: #e0f2f1; border-radius: 8px; padding: 15px; margin-top: 20px; margin-bottom: 20px; border: 2px solid #00796b; font-size: 1.1em; }\n");
    printf("        .counter-container strong { color: #e65100; }\n");
    // <pre> 태그의 가운데 정렬을 위해 수정
    ...
    printf("        }\n");
    printf("        .audio-stream-container {\n");
    printf("            width: 300px;\n");
    printf("            height: 67px; /* 200px의 1/3 */\n");
    printf("        }\n");
    printf("        .audio-stream {\n");
    printf("            width: 100%%;\n");
    printf("            height: 100%%;\n");
    ...
    printf("        .main-content, .chat-and-new-container { flex-grow: 1; }\n");
    ...
    // donation-result와 location-result의 min-height를 제거
    printf("        #donation-result, #location-result { \n");
    printf("            margin-top: 15px; padding: 15px; \n");
    printf("            background-color: #f5f5f5; border: 1px solid #ddd; \n");
    printf("            border-radius: 8px; \n");
    printf("            box-sizing: border-box; /* 패딩과 보더 포함 */\n");
    ...
    // 반응형 미디어 쿼리 추가: 화면이 768px보다 넓으면 가로 배치
    printf("        @media (min-width: 768px) {\n");
    printf("            .responsive-wrapper { flex-direction: row; align-items: flex-start; }\n");
    ...
    printf("            box-shadow: 0 2px 5px rgba(0,0,0,0.1); \n");
    printf("        }\n");
    printf("    </style>\n");
    printf("</head>\n");
    printf("<body>\n");
    ...
    // === 새로고침 안내 배너 ===
    printf("    <div class=\"refresh-banner\">\n");
    printf("        <p>새로고침후에는 <a href=\"#\" onclick=\"window.location.reload(); return false;\">오디오 재생 버튼</a>을 다시 눌러야 됩니다</p>\n");
    printf("    </div>\n");
    // ===========================

    // 메인 콘텐츠와 방문자 카운터, 채팅창을 감싸는 wrapper 시작
    printf("    <div class=\"responsive-wrapper\">\n");

    // 메인 콘텐츠
    printf("        <div class=\"main-content\">\n");
    printf("            <div class=\"container\">\n");
    printf("                <div class=\"stream-container\">\n");
    printf("                    <div class=\"image-stream-container\">\n");
    printf("                        <img id=\"liveImage\" class=\"image-stream\" src=\"\" alt=\"Live Feed\">\n");
    printf("                    </div>\n");
    printf("                    <div class=\"audio-stream-container\">\n");
    printf("                        <audio id=\"liveAudio\" class=\"audio-stream\" controls autoplay></audio>\n");
    printf("                    </div>\n");
    printf("                </div>\n");
    printf("            </div>\n");
    printf("            <div style=\"margin-bottom: 20px;\">\n");
    printf("                <a href=\"#\" id=\"location-link\">장소</a>\n");
    printf("            </div>\n");
    ...
    // 채팅창과 새로운 컨테이너를 함께 묶어서 관리
    printf("        <div class=\"chat-and-new-container\">\n");
    printf("            <div class=\"chat-wrapper\">\n");
    printf("                <iframe src=\"/chat.html\" frameborder=\"0\" style=\"width: 350px; height: 500px;\"></iframe>\n");
    printf("            </div>\n");
    // '새로운 컨테이너'를 빈 상태로 둡니다.
printf("            <div class=\"new-container\" id=\"new-container\"></div>\n");

// '새로운 컨테이너'를 활성화하는 버튼을 추가합니다.
printf("            <div style=\"text-align: center; margin-top: 10px;\">\n");
printf("                <button id=\"showShellResultBtn\">서버 정보 보기</button>\n");
printf("            </div>\n");
    printf("            <div class=\"new-container\" id=\"new-container\">\n");
    printf("                <h3>새로운 컨테이너</h3>\n");
    printf("                <p>이곳에 원하는 콘텐츠를 자유롭게 넣어보세요.</p>\n");
    printf("            </div>\n");
    ...
    printf("    </div>\n"); // 메인 컨텐츠 wrapper div 끝

    // === 푸터 영역은 전체 페이지의 가장 아래에 위치하도록 이동했습니다. ===
    ...
    printf("            var liveImage = document.getElementById('liveImage');\n");
    printf("            var imageUrl = 'http://125.251.58.88:8090/test.mpjpeg';\n");
    printf("            if (liveImage) {\n");
    printf("                setInterval(function() {\n");
    printf("                    liveImage.src = imageUrl + '?' + new Date().getTime();\n");
    printf("                }, 1000);\n");
    printf("            }\n");
    printf("\n");
    printf("            var liveAudio = document.getElementById('liveAudio');\n");
    printf("            var audioUrl = 'http://125.251.58.88:8090/test.wav';\n");
    printf("            if (liveAudio) {\n");
    printf("                liveAudio.src = audioUrl;\n");
    printf("            }\n");
    printf("        });\n");
    // ... (기존 HTML <body> 내용) ...
    printf("\n");
    printf("        document.getElementById('showShellResultBtn').addEventListener('click', function() {\n");
    printf("            var newContainer = document.getElementById('new-container');\n");
    printf("            if (newContainer.innerHTML.trim() !== '') {\n");
    printf("                newContainer.innerHTML = '';\n");
    printf("                return;\n");
    printf("            }\n");
    printf("            newContainer.innerHTML = '로딩 중...';\n");
    printf("            fetch('/cgi-bin/shell_result.cgi')\n");
    printf("                .then(response => response.text())\n");
    printf("                .then(data => {\n");
    printf("                    newContainer.innerHTML = data;\n");
    printf("                })\n");
    printf("                .catch(error => {\n");
    printf("                    newContainer.innerHTML = '<p style=\"color: red;\">데이터를 불러오는 데 실패했습니다.</p>';\n");
